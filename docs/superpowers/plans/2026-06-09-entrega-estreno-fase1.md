...
git commit -m "R123 — Infra: bucket R2 MEDIA + secrets de Stream para la entrega"
```
...
Create `worker/migrations/r113-entrega-manifiesto.sql`:
```sql
-- R123 — Entrega WOW "El Estreno": manifiesto, textos, estado del gate, estado de migración y video.
...
git commit -m "R123 — D1: columnas de entrega (manifiesto, textos, estados, video)"
```
...
git commit -m "R123 — Worker: helpers puros de media de entrega + tests"
```
...
git commit -m "R123 — Worker: ruta /media sirve fotos de R2 con Transformations"
```
...
```
// Ultima modificacion: <YYYY-MM-DD HH:MM:SS> CST (R123) — nueva función prepararCarpetaEntrega:
...
```markdown
## R123 — Entrega WOW "El Estreno" (Fase 1)
...
git commit -m "R123 — Adapter: prepararCarpetaEntrega (requiere despliegue manual)"
```
...
git commit -m "R123 — Worker: prepararEntrega migra fotos a R2 y video a Stream"
```
...
git commit -m "R123 — Worker: obtenerEntrega entrega el manifiesto al cliente"
```
...
git commit -m "R123 — Frontend: entrega.html (galería WOW desde R2/Stream, mobile-first)"
```
...
git commit -m "R123 — Admin: gate de configuración y publicación de la entrega"
```
...
git commit -m "R123 — Portal redirige a entrega.html en la etapa de entrega"
```
...
- Modify: `docs/RONDAS.md` (resumen R123)
...
Completa R123 en `docs/RONDAS.md` con todo lo construido y la nota de despliegue manual del adapter.

- [ ] **Step 3: Commit**
```bash
git add docs/ARQUITECTURA.md docs/RONDAS.md
git commit -m "R123 — Docs: entrega.html en mapa de relaciones y RONDAS"
```
