...
| `entrega.html` | Galería de entrega "El Estreno" (R113): Estreno, hero con video de Stream, fotos desde R2, 360, Kit, reseña, recontratación | `portal.js` | `obtenerEntrega` (lectura cliente). Las fotos se sirven por la ruta `/media/<key>` (R2 + Image Transformations). Gestión en admin: `prepararEntrega`, `guardarConfigEntrega`, `publicarEntrega` (en `contratos.js`). Adapter: `prepararCarpetaEntrega`. |
